    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        ...
        <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
          <el-table-column prop="voteTitle" label="标题"/>
          <el-table-column prop="voteDetails" label="详情"/>
          <el-table-column prop="voteAddress" label="下发地址">
            <template slot-scope="scope">
              <span>{{ parseAddress(scope.row.voteAddress) }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="voteStarttime" label="开始时间">
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.voteStarttime) }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="voteEndtime" label="结束时间">
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.voteEndtime) }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="voteAddress" label="地址"/>
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      this.url = 'aaa/voteAndApply/showVote'
      const sort = 'id,desc'
