  ...
  <div class="app-container">
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        ...
        <div class="head-container">
          <!-- 搜索 -->
          <el-input v-model="query.blurry" clearable placeholder="报名标题" style="width: 200px;" class="filter-item" @keyup.enter.native="toQuery"/>
        ...
        <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
          <el-table-column prop="applyTitle" label="标题"/>
          <el-table-column prop="applyDetails" label="详情"/>
          <el-table-column prop="applyAddress" label="下发地址">
            <template slot-scope="scope">
              <span>{{ parseAddress(scope.row.applyAddress) }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="applyStarttime" label="开始时间"/>
          <el-table-column prop="applyEndtime" label="结束时间"/>
          <el-table-column prop="applyAddress" label="地址"/>
          <el-table-column v-if="checkPermission(['ADMIN','USER_ALL','USER_EDIT','USER_DELETE'])" label="操作" width="125" align="center" fixed="right">
            <template slot-scope="scope">
              ...
              <el-popover
                ...
                :ref="scope.row.applyId"
                placement="top"
                width="180">
                <p>确定删除本条数据吗？</p>
                <div style="text-align: right; margin: 0">
                  <el-button size="mini" type="text" @click="$refs[scope.row.applyId].doClose()">取消</el-button>
                  <el-button :loading="delLoading" type="primary" size="mini" @click="subDelete(scope.row.applyId)">确定</el-button>
                </div>
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      this.url = 'aaa/voteAndApply/showApply'
      const sort = 'id,desc'
    ...
    edit(data) {
      ...
      _this.form = data
      // _this.roleIds = []
      // _this.form = { id: data.id, username: data.username, phone: data.phone, email: data.email, roles: [], dept: { id: data.dept.id }, job: { id: data.job.id }}
      _this.dialog = true
    }
  }
}
